"""Collect all simulated fields of a split into one npz for selector training/evaluation."""
...
def main():
    ...
    for d in insts:
        try:
            ...
            print("SKIP", d, e)
    out = os.path.join(split_dir, "fields.npz")
    ...
    print(f"wrote {ok}/{len(insts)} {args.split} -> {out}  fields {np.stack(fields).shape} "
